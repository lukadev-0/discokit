...
Discokit is a library that allows you to build Discord bots. At its core, Discokit
is a wrapper around the [Discord API](https://discord.com/developers/docs/intro).
...
<Tabs items={["npm", "pnpm", "yarn"]}>
  <Tab>```bash npm i discokit ```</Tab>
  <Tab>```bash pnpm i discokit ```</Tab>
  <Tab>```bash yarn add discokit ```</Tab>
</Tabs>
...
<Tabs items={["npm", "pnpm", "yarn"]}>
  <Tab>```bash npm start ```</Tab>
  <Tab>```bash pnpm start ```</Tab>
  <Tab>```bash yarn start ```</Tab>
</Tabs>
